...
const SupplierAdd = () => {
  ...
  const cardTitle = params.id ? 'supplier edit' : 'supplier add';
  ...
  const saveSupplier = async (data) => {
    try {
      await SupplierService.saveSupplier(data);
      navigate('/store/supplier/list');
      notification['success']({ message: 'Supplier saved successfully' });
      form.resetFields();
    } catch (error) {
  ...
  const onFinish = (values) => {
    ...
    params.id ? updateSupplier(params.id, values) : saveSupplier(values);
    console.log(values);
  };
  ...
  useEffect(() => {
    ...
    params.id && getSupplierById(params.id);
    getAllCity();
    getAllCountry();
  }, [params.id]);
    ...
    <Form.Item name="prefix" noStyle>
      ...
      >
        {countries.map((country) => {
          return (
            <Option value={country.dialingCode}>{country.dialingCode}</Option>
          );
        })}
      </Select>
    </Form.Item>
    ...
    <CommonLayout>
      <ARMBreadCrumbs>
        <Breadcrumb separator="/">
          ...
          <Breadcrumb.Item>
            <Link to="/store/supplier/list">Suppliers</Link>
          </Breadcrumb.Item>
          <Breadcrumb.Item>Supplier Add</Breadcrumb.Item>
        </Breadcrumb>
      </ARMBreadCrumbs>

      <ARMCard title={getLinkAndTitle(cardTitle, '/store/supplier/list')}>
        <ARMForm
          {...layout}
          form={form}
          name="control-hooks"
